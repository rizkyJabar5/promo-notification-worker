...
public interface MasterPromoRepository extends ReadOnlyRepository<MasterPromo, String> {
    @Query("""
            select distinct new id.co.bni.surrounding.promo.workernotification.additional.projection.PromoProjection(u.userId, mp.promoId, mp.type)
            from MasterPromo mp
            join PromoBrand pb
                on upper(pb.brand) = upper(mp.brand)
                and pb.category = mp.category
